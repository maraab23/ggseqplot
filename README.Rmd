...
This package literally builds on the excellent work of the `{TraMiner}`, `{TraMineRextras}`, and {ggplot2} developers and uses several of the their functions to produce {ggplot2}-flavored figures.

The development of this library is in an early stage and will be hopefully complemented by a few
...
frequency group and display the resulting relative frequency sequence plots. 
Before doing so we first have to compute a dissimilarity matrix:

```{r diss}
diss <- seqdist(biofam.seq, method="LCS")
```

```{r seqplot.rf1, out.width="80%"}
# with TraMineRextras::seqplot.rf
seqplot.rf(biofam.seq, diss = diss, k = 20)
```

```{r ggseqrfplot1, out.width="80%"}
...
line in a common plot and thus resembles the new `{TraMineRextras}` function `seqplot.tentrop`.
